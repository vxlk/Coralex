...
std::vector<Image*> ImageFactory::getImages(const std::string& name) {
  ...
  //RenderContext &renderContext  = RenderContext::getInstance(); //you don't need this
  ...
  for (unsigned i = 0; i < numberOfFrames; ++i) {
    ...
    SDL_Texture* texture = 
      SDL_CreateTextureFromSurface(RenderContext::getInstance().getRenderer(),surface);
