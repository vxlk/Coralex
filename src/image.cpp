...
void Image::regenerateTexture(){
  ...
  texture = SDL_CreateTextureFromSurface(RenderContext::getInstance().getRenderer(), surface);
}
...
//rotation
void Image::drawRotation(int x, int y, float scale, double rotation) const
{
  x -= Viewport::getInstance().getX();
  y -= Viewport::getInstance().getY();
  int tempHeight = scale*view.h;
  int tempWidth =  scale*view.w;
  SDL_Rect dest =  {x, y, tempWidth, tempHeight};
  SDL_RenderCopyEx(renderer, texture, &view, &dest, rotation, nullptr, SDL_FLIP_NONE);
}
